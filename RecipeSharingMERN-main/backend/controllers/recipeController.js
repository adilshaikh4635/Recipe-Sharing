...
const getRecipes = async (req, res) => {
  ...
  try {
    ...
    const recipesWithUserDetails = await Promise.all(recipes.map(async (recipe) => {
      ...
      const username = user ? user.username : 'Unknown'; // Handle case if user is not found
      return {
        ...recipe._doc,
        username
      };
...
const editAdminRecipe = async (req, res) => {
  try {
      ...
      if (!recipeId) {
          return res.status(400).json({ message: "Recipe ID is missing in the request URL" });
      }
