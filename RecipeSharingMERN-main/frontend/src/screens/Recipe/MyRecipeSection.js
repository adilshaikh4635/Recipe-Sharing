...
const MyRecipeSection = () => {
  ...
  useEffect(() => {
    console.log("Recipes State:", recipes); // Debugging log
  }, [recipes]);
  ...
  const deleteRecipe = async (recipeId) => {
    try {
      ...
      console.error("Error deleting recipe:", error); // Debugging log
  ...
  const handleUpdate = async (values) => {
    try {
      ...
      console.error("Error updating recipe:", err); // Debugging log
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h1>My Recipes</h1>
      {recipes.length === 0 && <p>No recipes found. Add a new recipe to get started!</p>}
